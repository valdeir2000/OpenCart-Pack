...
class ModelInstall extends Model {
	public function database($data) {
		...
		if ($lines) {
			...
			$db->query("INSERT INTO `" . $data['db_prefix'] . "user` SET user_id = '1', user_group_id = '1', username = '" . $db->escape($data['username']) . "', salt = '" . $db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', firstname = 'Valdeir', lastname = 'Santana', email = '" . $db->escape($data['email']) . "', status = '1', date_added = NOW()");
